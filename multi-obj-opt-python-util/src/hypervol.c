...
void populateFront(double  **front1[], double* A, int sizeFront1, int noObjectives)
{
	*front1 = malloc(sizeFront1 * sizeof(double *));
	if (*front1 == NULL)  ERROR("memory allocation failed");
	for (int i = 0; i < sizeFront1; i++) {
		(*front1)[i] = malloc(noObjectives * sizeof(double));
		if ((*front1)[i] == NULL)  ERROR("memory allocation failed");
		for(int j = 0; j < noObjectives; j++)
			(*front1)[i][j] = A[i * noObjectives + j];
	}
}
